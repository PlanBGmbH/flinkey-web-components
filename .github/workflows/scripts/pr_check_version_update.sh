...
echo "$( jq '.version = "$PR_PACKAGE_VERSION"' ./pr/components/package.json )" > ./pr/components/package.json
echo "$( jq '.version = "$PR_PACKAGE_VERSION"' ./pr/components/package-lock.json )" > ./pr/components/package-lock.json

if [ $changes -eq 1 ];
then
    echo 'components directory content changed'
    echo "PR package version: $PR_PACKAGE_VERSION"
    echo "Main package version: $mainPackageVersion"
    if $(dpkg --compare-versions $PR_PACKAGE_VERSION gt $mainPackageVersion);
    then
    ...
    if $(dpkg --compare-versions $PR_PACKAGE_VERSION ne $mainPackageVersion);
    then
